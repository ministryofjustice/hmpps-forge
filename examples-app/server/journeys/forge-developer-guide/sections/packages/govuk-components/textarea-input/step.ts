...
export const textareaInputStep = step({
  path: '/textarea-input',
  title: 'Textarea',
  ...
  onAccess: [
    access({
      effects: [GuideEffects.LoadContent('govuk-textarea-input')],
    }),
  ],
  blocks: [content],
  onSubmission: [
    exampleSubmit(EXAMPLE_GROUPS.basic),
    exampleSubmit(EXAMPLE_GROUPS.description),
    exampleSubmit(EXAMPLE_GROUPS.notes),
  ],
})
