...
/** Seeds draft answers so the demo can start at different resume frontiers. */
export const seedDraftAnswers = effect({
  name: 'SeedDraftAnswers',
  factory: () => (context: PatternEffectContext, answers: Record<string, unknown>) => {
    const session = context.getSession()

    if (session) {
      session.draftAnswers = { ...session.draftAnswers, ...answers }
    }
