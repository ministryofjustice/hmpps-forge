...
describe('singleQuestionPerPageDemoJourney', () => {
  beforeEach(() => {
    vi.resetAllMocks()
  ...
  it('should save confirmed answers outside the session when the visitor confirms', async () => {
    ...
    // Act
    const result = await client.post('/single-question-per-page/check-answers', { session, body: {} })

    // Assert
    expect(answerStore.save).toHaveBeenCalledWith(answers)
    expect(session).toEqual({})
    expect(result).toMatchObject({ type: 'redirect', url: '/single-question-per-page/confirmation' })
  })
  ...
  it('should keep draft answers in the session when a question is submitted', async () => {
    ...
    // Act
    await client.post('/single-question-per-page/your-name', { session, body: { fullName: 'Jane Smith' } })
  ...
  it('should delete the saved record when the existing restart action clears answers', async () => {
    ...
    // Act
    await client.post('/single-question-per-page/confirmation', { session, body: { action: 'restart' } })
  ...
  it('should show confirmation without session data when a saved record exists', async () => {
    ...
    // Act
    const result = await client.get('/single-question-per-page/confirmation', { session: {} })
  ...
  it('should keep confirmation unreachable when no saved record exists', async () => {
    ...
    expect(result).not.toMatchObject({ url: '/single-question-per-page/confirmation' })
  })
  ...
  it('should make confirmation unreachable when the saved record is deleted on restart', async () => {
    ...
    // Act
    const restart = await client.post('/single-question-per-page/confirmation', { session, body: { action: 'restart' } })
    const confirmation = await client.get('/single-question-per-page/confirmation', { session })

    // Assert
    expect(answerStore.delete).toHaveBeenCalledWith()
    expect(restart).toMatchObject({ type: 'redirect', url: '/single-question-per-page/overview' })
    expect(confirmation).toMatchObject({ type: 'redirect' })
    expect(confirmation).not.toMatchObject({ url: '/single-question-per-page/confirmation' })
  })
})
