...
Runtime applies those decisions to a specific request, response, session,
answers, data, route params, and query string.

This phase does not rebuild the journey structure. It prepares context, runs
compiled functions, evaluates navigation, and delegates rendering or redirect
handling to the framework adapter.

## Why runtime stays plan-driven

Runtime is the request path. It should do the minimum work needed for the
current request.

Forge keeps runtime plan-driven so handlers do not need to inspect the original
DSL. The route handler already knows which step or journey-root plan it owns.
It can call the compiled functions attached to that plan, then use ordinary
runtime code for policy decisions such as redirect handling, reachability graph
...
It is the point where the compiled journey meets an incoming HTTP request.

It starts when `ForgeRouter` mounts route handlers through the framework
adapter. Step routes receive GET and POST handlers. Journey roots receive a GET
handler used for entry and resume-style navigation.

The mounted handlers do not compile the journey. They receive the plans and
compiled functions produced by phase 3 and apply them to the current request.

Runtime has three route-level entry points:

- step GET requests, which render a step
- step POST requests, which process submitted data for a step
- journey-root GET requests, which redirect into the correct step

The sections below describe those flows.

## GET step requests

A GET request renders a step.

Forge starts by converting the framework request and response into Forge request
objects. It then creates a `RuntimeEvaluationContext` for the request and merges
static journey data into that context.

Forge runs the compiled access lifecycle before doing any step evaluation. If an
access hook returns a redirect, Forge redirects immediately. If it returns an
error, Forge raises an HTTP error.
...
not be served, Forge redirects to the correct route. This can happen when resume
navigation is active, or when the current step is no longer reachable.
...
and passes that context to the framework adapter.

## POST step requests

A POST request processes submitted data for a step.

Forge begins the same way as a GET request. It converts the framework request
and response, prepares the runtime context, and runs the compiled access
lifecycle.
...
reachable, Forge redirects to the correct route before running submit hooks.
...
If a submit hook redirects, Forge delegates that redirect to the framework
adapter. If it returns an error, Forge raises an HTTP error.

If the request is not redirected, Forge renders the current step. Validation
failures from the submit lifecycle are attached to the render context so the
adapter can render the response with errors.
...
- the request and response from the framework adapter
...
- a redirect
- an HTTP error
- a render context passed to the framework adapter
...
`ForgeRouter` mounts the compiled journey into the host framework.

It builds route-template catalogs, creates journey routers, mounts step GET and
POST handlers, and mounts journey-root GET handlers. It also records navigation
metadata used later when render contexts are built.
...
The framework adapter owns the final response. Forge builds the render context;
the adapter decides how that context becomes an HTTP response.
...
- the framework adapter cannot render or redirect the response
...
After runtime finishes a request, Forge has either rendered a response,
redirected to another route, or raised an error for the framework to handle.
