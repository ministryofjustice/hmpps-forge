...
import AndPredicateHandler from './AndPredicateHandler'

describe('AndPredicateHandler', () => {
  beforeEach(() => {
  ...
  describe('evaluate()', () => {
    it('should return true for primitive truthy values when all operands are truthy', async () => {
      ...
      const handler = new AndPredicateHandler(andPredicate.id, andPredicate as any)
    ...
    it('should handle nested AND predicates correctly', async () => {
      // Arrange
      const innerAnd = ASTTestFactory.predicate(LogicType.AND)
      const operand1 = ASTTestFactory.predicate(LogicType.TEST)
      const outerAnd = ASTTestFactory.predicate(LogicType.AND, {
        operands: [innerAnd, operand1, true] as any,
      })
      const handler = new AndPredicateHandler(outerAnd.id, outerAnd as any)
      const mockContext = createMockContext()
      const mockInvoker = createSequentialMockInvoker([true, true])
