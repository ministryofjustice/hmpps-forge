 ...
 *
 * All handlers are HybridThunkHandler which can operate in sync or async mode:
 * - isAsync = false: evaluateSync() is called directly, no Promise overhead
 * - isAsync = true: evaluate() is called with full async support
 *
 * The evaluator detects sync handlers and fast-paths them for performance.
 *
