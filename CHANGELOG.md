...
- **`createTestEffectContext` builds a real effect context for tests.** Effect tests
  ...
  happened to touch, drifting from the real class as it gained read-only proxies and
  mutation history. `createTestEffectContext({ answers, data, session, ... })` returns a
  genuine `EffectFunctionContext` over in-memory state, plus `getResponseHeaders()` and
