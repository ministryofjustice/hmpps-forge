...
describe('createNextForgePage', () => {
  ...
  it('should reject components that render a plain function component when used with server actions', async () => {
    // Arrange
    interface BadBlock extends BlockDefinition, BasicBlockProps {
      variant: 'bad'
    }

    const Inner = () => <p>hi</p>
    const badComponent = buildReactComponent<BadBlock>('bad', () => <Inner />)
    ...
    const badJourney = journey({
      ...
      steps: [
        step({
          ...
          path: '/start',
          title: 'Start',
          blocks: [block<BadBlock>({ variant: 'bad' })],
        }),
      ],
    })

    const forge = new Forge({ logger: silentLogger, basePath: '/forms' })
      .registerGlobalComponent(badComponent)
      .registerPackage({ journey: badJourney })
    ...
    // Act & Assert
    await expect(Page({ params: { forgePath: ['demo', 'start'] } })).rejects.toThrow(
      'A Forge component rendered <Inner>',
    )
  })
})
