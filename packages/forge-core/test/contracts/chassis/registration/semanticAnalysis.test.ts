...
const IsYes = condition('Semantic.IsYes', { factory: () => (value: unknown) => value === 'yes' })
...
describe('semantic analysis contracts', () => {
  describe('reference scopes', () => {
    ...
    it('should reject a parent item reference when it exceeds the available iterator depth', () => {
      // Arrange
      const Emit = effect('Semantic.Emit', { factory: () => (_context, _value: unknown) => undefined })
      const journeyDefinition = journeyWithFields([], {
        onAccess: [
          access({
            effects: [Emit(Data('items').each(Iterator.Map(Format('%1', Loop.Parent.Item().path('name')))))],
          }),
        ],
      })
  ...
  describe('placement rules', () => {
    it('should reject an effect when placed outside a hook', () => {
      // Arrange
      const Track = effect('Semantic.Track', { factory: () => () => undefined })
      const journeyDefinition = journeyWithFields([
  ...
  describe('function arity', () => {
    it('should reject a function call when its arguments violate the registered tuple schema', () => {
      // Arrange
      const HasMinLength = condition('Semantic.HasMinLength', {
