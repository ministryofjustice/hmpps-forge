...
export const iteratorPredicatesJourney = evaluationJourney('iterator-predicates', {
  count: Data('items').each(Iterator.Count(Loop.Item().value().match(Condition.Equals('yes')))),
  asyncCount: Data('items').each(Iterator.Count(Loop.Item().value().match(AsyncExpressionConditions.IsYesAsync()))),
  countPlusOne: Data('items')
    .each(Iterator.Count(Loop.Item().value().match(Condition.Equals('yes'))))
      ...
      Loop.Item()
        ...
        .each(
          Iterator.Count(
            Loop.Item()
              .value()
              .match(Condition.Equals(Loop.Parent.Item().path('expected'))),
          ),
        ),
    ),
  ),
  keyedCount: Data('keyed').each(Iterator.Count(Loop.Item().value().match(Condition.Number.GreaterThan(0)))),
  some: Data('items').each(Iterator.Some(Loop.Item().value().match(Condition.Equals('yes')))),
  every: Data('items').each(Iterator.Every(Loop.Item().value().match(Condition.Equals('yes')))),
  asyncSome: Data('items').each(Iterator.Some(Loop.Item().value().match(AsyncExpressionConditions.IsYesAsync()))),
  asyncEvery: Data('items').each(Iterator.Every(Loop.Item().value().match(AsyncExpressionConditions.IsYesAsync()))),
  composed: when(
    and(
      Data('items').each(Iterator.Some(Loop.Item().value().match(Condition.Equals('yes')))),
      not(Data('items').each(Iterator.Every(Loop.Item().value().match(Condition.Equals('yes'))))),
    ),
  )
    .then('mixed')
    .else('uniform'),
  ...
  keyed: Data('keyed').each(Iterator.Some(Loop.Item().key().match(Condition.Equals('target')))),
})

const FailOnUnexpectedItem = condition('Expressions.FailOnUnexpectedItem', {
...
export const someShortCircuitJourney = evaluationJourney('some-short-circuit', {
  result: Data('items').each(Iterator.Some(Loop.Item().value().match(FailOnUnexpectedItem()))),
})

export const everyShortCircuitJourney = evaluationJourney('every-short-circuit', {
  result: Data('items').each(Iterator.Every(Loop.Item().value().match(FailOnUnexpectedItem()))),
})
