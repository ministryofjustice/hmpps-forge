...
import { createClient, ContractGenerators, Effects } from '../../contractHelpers'
...
describe('expression contracts', () => {
  ...
  describe('nullish()', () => {
    it.each([
      ...
      async ({ primaryValue, expected, fallbackCalls }) => {
        ...
        const Primary = ContractGenerators.register(() => primary)
        const Fallback = ContractGenerators.register(() => fallback)
    ...
    it('should compose across value builders when values and branches are missing', async () => {
      ...
      const Unused = ContractGenerators.register(() => unused)
      ...
      const client = createClient(
        evaluationJourney('fluent-nullish', {
          ...
          items: Data('items').each(Iterator.Map(Item().nullish('item'))),
          scopedItems: Data('items').each(Iterator.Map(Item().nullish('item'))),
          collection: Data('empty').each(Iterator.Map(Item().value())).nullish(['fallback']),
          found: Data('empty').each(Iterator.Find(Item().value().match(GeneralConditions.Equals(true))))
            .nullish({ name: 'found fallback' }).path('name'),
          count: Data('empty').each(Iterator.Count(Item().value().match(GeneralConditions.Equals(true)))).nullish(100),
        }),
      )
    ...
    it('should propagate the primary error when a fallback is available', async () => {
      // Arrange
      const Primary = ContractGenerators.register('FailingPrimary', {
        factory: () => () => {
  ...
  describe('native match cases', () => {
    ...
    it('should share the subject across combinator leaves while keeping argument predicates independent', async () => {
      ...
      const Subject = ContractGenerators.register(() => subject)
      const client = createClient(
    ...
    it('should evaluate the subject once and keep ordered mixed branches lazy', async () => {
      ...
      const Subject = ContractGenerators.register(() => subject)
      const Selected = ContractGenerators.register(() => selected)
      const Unused = ContractGenerators.register(() => unused)
      const client = createClient(
  ...
  describe('component properties and guards', () => {
    it('should resolve language helpers when they appear in component properties and visibility guards', async () => {
      // Arrange
      const subject = vi.fn(async () => 'selected')
      const unused = vi.fn(async () => {
        throw new Error('Unselected property evaluated')
      })
      const Subject = ContractGenerators.register(() => subject)
      const Unused = ContractGenerators.register(() => unused)
      const isYes = Item().value().match(GeneralConditions.Equals('yes'))
      const some = Data('items').each(Iterator.Some(isYes))
      const every = Data('items').each(Iterator.Every(isYes))
      const client = createClient(
        journey({
          ...
          steps: [
            step({
              path: '/result',
              title: Data('missing').nullish('Result'),
              view: { locals: { some, every } },
              ...
              blocks: [
                ...
                StaticText({ text: Data('missing').nullish('fallback'), visibleWhen: Data('visible').nullish(true) }),
                StaticText({ text: Data('items').each(Iterator.Count(isYes)) }),
                StaticText({ text: 'hidden', visibleWhen: every }),
                StaticText({ text: when(some).then('yes').else(Unused()).pipe(StringTransformers.ToUpperCase()) }),
                StaticText({
                  text: Data('items').each(Iterator.Map(match(Item().value()).case('yes', 'Y').otherwise('N'))),
                }),
                StaticText({ text: Data('items').each(Iterator.Filter(isYes)).each(Iterator.Count(isYes)) }),
              ],
            }),
          ],
        }),
      ...
      // Act
      const result = await client.get('/language-properties/result', {
        session: { data: { items: ['no', 'yes', null, undefined] } },
      })
      ...
      if (result.type === 'render') {
        expect(result.context.step.title).toBe('Result')
        expect(result.context.step.view).toMatchObject({ locals: { some: true, every: false } })
        expect(
          result.context.blocks
            .filter(block => block.properties.visibleWhen !== false)
            .map(block => block.properties.text),
        ).toEqual(['chosen', 'fallback', 1, 'YES', ['N', 'Y'], 1])
      }
    })

    it('should retain item skipping and count against the request budget when using the new iterators', async () => {
      // Arrange
      const predicate = Item().value().match(GeneralConditions.Equals('yes'))
      const definition = evaluationJourney('count-budget', { count: Data('items').each(Iterator.Count(predicate)) })
      const client = createClient(definition, { maxIteratorIterations: 1 })

      // Act
      const withinBudget = await client.get('/count-budget/result', {
        session: { data: { items: [null, 'yes', undefined] } },
      })
      const overBudget = await client.get('/count-budget/result', { session: { data: { items: ['yes', 'no'] } } })

      // Assert
      expect(withinBudget.type).toBe('render')

      if (withinBudget.type === 'render') {
        expect(withinBudget.context.data.count).toBe(1)
      }

      expect(overBudget.type).toBe('error')

      if (overBudget.type === 'error') {
        expect(overBudget.error.message).toContain('exceeded the per-request limit of 1 iterations')
      }
    })
  })
