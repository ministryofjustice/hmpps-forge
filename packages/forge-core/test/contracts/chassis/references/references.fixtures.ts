 ...
 */
export const CaptureValue = effect('CaptureValue', {
    ...
    access({
      effects: [
        ...
        CaptureValue(
          'foundName',
          Data('people')
            .each(Iterator.Find(Loop.Item().path('id').match(Condition.Equals(2))))
            .path('name'),
        ),
        CaptureValue(
          'missingName',
          Data('people')
            .each(Iterator.Find(Loop.Item().path('id').match(Condition.Equals(99))))
            .path('name'),
        ),
      ],
    }),
  ...
  onAccess: [
    access({
      effects: [
        ...
        CaptureValue(
          'arrayKeys',
          Data('letters').each(Iterator.Map(Format('%1:%2', Loop.Item().key(), Loop.Item().value()))),
        ),
      ],
    }),
  ],
