...
const TrimAsync = transformer('Prep.TrimAsync', {
  inputSchema: z.string(),
  factory: () => async (value: string) => value.trim(),
})

const ToUpperAsync = transformer('Prep.ToUpperAsync', {
  inputSchema: z.string(),
  factory: () => async (value: string) => value.toUpperCase(),
})

const DefaultNameAsync = generator('Prep.DefaultNameAsync', {
  factory: () => async () => 'generated-name',
})

const EqualsAsync = condition('Prep.EqualsAsync', {
