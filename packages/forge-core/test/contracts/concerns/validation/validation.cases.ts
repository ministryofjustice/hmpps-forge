...
const RejectValue = generator('RejectValue', {
  factory: () => (value: ResolvableValue, forbidden: ResolvableValue) =>
    value === forbidden ? [{ message: `Value cannot be ${String(forbidden)}` }] : undefined,
})
const RejectValueAsync = generator('RejectValueAsync', {
  factory: () => async (value: ResolvableValue, forbidden: ResolvableValue) =>
    value === forbidden ? [{ message: `Value cannot be ${String(forbidden)}` }] : undefined,
})
...
export const cases: StepContractCase[] = [
      ...
      blocks: [
        TextField({
          ...
          validWhen: Data('forbiddenWords').each(
            Iterator.Map(
              validation({
                condition: Self().not.match(Condition.Equals(Item())),
  ...
  {
    description: 'function validation rules',
    step: {
      blocks: [
        TextField({
          code: 'keyword',
          validWhen: [validation({ function: RejectValue(Self(), 'nope') })],
        }),
      ],
    },
    tests: [
      {
        name: 'should surface errors returned by a validation function',
        post: { keyword: 'nope' },
        errors: { keyword: ['Value cannot be nope'] },
      },
    ],
  },
  {
    description: 'async function validation rules',
    step: {
      blocks: [
        TextField({
          code: 'keyword',
          validWhen: [validation({ function: RejectValueAsync(Self(), 'nope') })],
        }),
      ],
    },
    tests: [
      {
        name: 'should surface errors returned by an async validation function',
        post: { keyword: 'nope' },
        errors: { keyword: ['Value cannot be nope'] },
      },
      {
        name: 'should pass when an async validation function returns no errors',
        post: { keyword: 'fine' },
        valid: true,
      },
    ],
