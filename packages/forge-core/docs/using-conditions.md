...
```typescript
import { and, or, xor, not } from '@ministryofjustice/hmpps-forge/core/authoring'
```
...
If a condition does not need dependencies, omit the deps type parameter.
...
export const { conditions: MyConditions, implementations: MyConditionsImplementations } =
  defineConditionFunctions<..., MyDeps>({
    IsPrisonNumber: () => (value: unknown) => {
      assertString(value, 'MyConditions.IsPrisonNumber')
      return /^[A-Z][0-9]{4}[A-Z]{2}$/i.test(value.trim())
    },

    IsValidPostcode: ({ apiClient }) => async (value: unknown) => {
      assertString(value, 'MyConditions.IsValidPostcode')
      const result = await apiClient.validatePostcode(value)
...
export default createForgePackage<MyDeps>({
  journey: myJourney,
  functions: {
    ...MyConditionsImplementations,
  },
})

// At startup
forge.registerPackage(myForgePackage, { apiClient })
```
...
```typescript
import { assertString, assertNumber } from '@ministryofjustice/hmpps-forge/core/authoring'
