...
References are the foundation of forge's data access layer. They provide a declarative way to point to different sources of data within your forms, from field values to external data and HTTP request information.

## What is a Reference?

A reference is a pointer to data that gets resolved at runtime. Instead of hardcoding values, you describe *where* the value comes from, and forge resolves it when needed.
...
Forge provides these reference types, each pointing to a different data source:
...
const { effects: MyEffects, implementations } =
  defineEffectFunctions<..., { api: ApiService }>({
    loadUserProfile: ({ api }) => async (context: EffectFunctionContext) => {
...
When forge compiles your form definition, it transforms `Self()` into an `Answer()` reference with the field's actual code:
