...
If a transformer does not need dependencies, omit the deps type parameter.
...
export const { transformers: MyTransformers, implementations: MyTransformersImplementations } =
  defineTransformerFunctions<..., MyDeps>({
    NormalisePrisonNumber: () => (value: unknown) => {
      assertString(value, 'MyTransformers.NormalisePrisonNumber')
      return value.replace(/\s/g, '').toUpperCase()
    },

    FormatCurrency: ({ formatter }) => (value: unknown) => {
