...
If a generator does not need dependencies, omit the deps type parameter.
...
export const { generators: MyGenerators, implementations: MyGeneratorsImplementations } =
  defineGeneratorFunctions<..., MyDeps>({
    UUID: () => () => crypto.randomUUID(),

    PrefixedUUID: () => (prefix: string) => `${prefix}${crypto.randomUUID()}`,

    // Sync generator with deps
    CurrentUserId: ({ userService }) => () => userService.getCurrentUserId(),

    // Async generator with deps
    ServerTime: ({ timeApi }) => async () => {
