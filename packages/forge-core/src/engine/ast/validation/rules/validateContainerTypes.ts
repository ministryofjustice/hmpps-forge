...
function checkEntries(
  entries: unknown,
  check: ContainerCheck,
  containerMetadata: DSLSourceMetadata | undefined,
  errors: Error[],
): void {
  ...
  entries.forEach((entry: unknown) => {
    ...
    const entryMetadata = isObject(entry) ? getDSLSourceMetadata(entry) : undefined

    errors.push(buildError(check, entryMetadata ?? containerMetadata))
  })
}
...
function checkHookBranch(
  ...
  checkEntries(branch.effects, EFFECTS, containerMetadata, errors)
  checkEntries(branch.next, NEXT, containerMetadata, errors)
}
...
export const validateContainerTypes: ASTValidationRule = (context: ASTValidationContext): readonly Error[] => {
  ...
  nodeIndex.findByType<StepASTNode>(ASTNodeType.STEP).forEach(step => {
    const metadata = getDSLSourceMetadata(step)

    checkEntries(step.properties.onAccess, ON_ACCESS, metadata, errors)
    checkEntries(step.properties.onSubmission, ON_SUBMISSION, metadata, errors)
    checkEntries(step.properties.blocks, BLOCKS, metadata, errors)
  })

  nodeIndex.findByType<JourneyASTNode>(ASTNodeType.JOURNEY).forEach(journey => {
    checkEntries(journey.properties.onAccess, ON_ACCESS, getDSLSourceMetadata(journey), errors)
  })

  nodeIndex.findByType<AccessHookASTNode>(HookType.ACCESS).forEach(hook => {
    const metadata = getDSLSourceMetadata(hook)

    checkEntries(hook.properties.effects, EFFECTS, metadata, errors)
    checkEntries(hook.properties.next, NEXT, metadata, errors)
  })

  nodeIndex.findByType<SubmitHookASTNode>(HookType.SUBMIT).forEach(hook => {
    const metadata = getDSLSourceMetadata(hook)

    checkHookBranch(hook.properties.onAlways, metadata, errors)
    checkHookBranch(hook.properties.onValid, metadata, errors)
    checkHookBranch(hook.properties.onInvalid, metadata, errors)
  })
