...
| [`ScopedComponentRegistry`](./ScopedComponentRegistry.ts) | A read-only view of `ComponentRegistry` | Returned on the `ForgeOutcome` so the adapter can resolve components during rendering |
