...
describe('CodeEmitter', () => {
  ...
  describe('comment()', () => {
    it('should emit line comments at the current indentation', () => {
      ...
      emitter.if('condition', () => emitter.comment('StepRenderCompiler.compileFieldValueResolution'))

      // Assert
      expect(emitter.toString()).toBe(
        [
          '// --- StepRenderCompiler.compileBlock ---',
          'if (condition) {',
          '',
          '  // --- StepRenderCompiler.compileFieldValueResolution ---',
          '}',
        ].join('\n'),
      )
    })
