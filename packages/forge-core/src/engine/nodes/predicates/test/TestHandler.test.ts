import { PredicateType, FunctionType } from '../../../../authoring/types/enums'
...
describe('TestHandler', () => {
  ...
  /**
   * Helper to create a TestHandler with default or custom options
   */
  function createTestHandler(
    ...
    const testPredicate = ASTTestFactory.predicate(PredicateType.TEST, {
      subject,
      condition,
      negate,
    })

    return new TestHandler(testPredicate.id, testPredicate as any)
  }
  ...
  describe('evaluate()', () => {
    it.each([
      ...
      // Arrange
      const handler = createTestHandler({ negate })
