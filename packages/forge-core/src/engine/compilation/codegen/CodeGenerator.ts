...
import { Code, SafeCode } from './Code'
import CommentCodeNode from './CommentCodeNode'
import ContinueCodeNode from './ContinueCodeNode'
import DeclarationCodeNode, { DeclarationKind } from './DeclarationCodeNode'
...
import IfCodeNode, { IfCodeBranch } from './IfCodeNode'
...
/** Builds structured generated-code nodes while owning names and lexical scope. */
export default class CodeGenerator {
  private readonly rootNodes: GeneratedCodeNode[] = []
  ...
  static forFunction(parameterPrefixes: readonly string[]): CodeGenerator {
    const parameters = parameterPrefixes.map(parameter => new Name(parameter))
  ...
  nextName(prefix = '_value'): Name {
    return new Name(`${prefix}${this.variableCounter++}`)
  }
  ...
  var(prefix: string, value?: SafeCode): Name {
  ...
  declareVar(name: Name, value?: SafeCode): void {
    this.reserveFunctionName(name)
    this.currentBody.push(new DeclarationCodeNode(DeclarationKind.VAR, name, value))
  }

  let(prefix: string, value?: SafeCode): Name {
  ...
  declareLet(name: Name, value?: SafeCode): void {
    this.reserveLexicalName(name)
    this.currentBody.push(new DeclarationCodeNode(DeclarationKind.LET, name, value))
  }

  const(prefix: string, value: SafeCode): Name {
    const name = this.allocateLexicalName(prefix)
  ...
  declareConst(name: Name, value: SafeCode): void {
    this.reserveLexicalName(name)
    this.currentBody.push(new DeclarationCodeNode(DeclarationKind.CONST, name, value))
  }
  ...
  forRange(prefix: string, from: SafeCode, to: SafeCode, body: (index: Name) => void): Name {
    const loopBody: GeneratedCodeNode[] = []
  ...
  tryCatch(tryBody: () => void, catchPrefix: string, catchBody: (error: Name) => void): Name {
    const compiledTryBody = this.captureLexicalBody(tryBody)
    const compiledCatchBody: GeneratedCodeNode[] = []
  ...
  function(
    ...
    buildBody: (generator: CodeGenerator, parameters: readonly Name[]) => void,
  ...
  functionExpression(
    ...
    return Code.functionExpression(
  ...
  toNodes(): readonly GeneratedCodeNode[] {
    return this.rootNodes
  }

  private captureLexicalBody(body: () => void): GeneratedCodeNode[] {
    const nodes: GeneratedCodeNode[] = []
  ...
  private allocateLexicalName(prefix: string): Name {
  ...
  private allocateFunctionName(prefix: string): Name {
    return this.allocateName(
      prefix,
      name => this.isNameVisible(name),
      name => {
  ...
  private allocateName(
    ...
    const baseName = new Name(prefix)
    ...
    let suffix = 1
    let candidate = new Name(`${prefix}_${suffix}`)

    while (isUnavailable(candidate.value)) {
      suffix += 1
      candidate = new Name(`${prefix}_${suffix}`)
    }
  ...
  private reserveLexicalName(name: Name): void {
  ...
  private reserveFunctionName(name: Name): void {
    if (this.isNameVisible(name.value)) {
  ...
  private get currentBody(): GeneratedCodeNode[] {
