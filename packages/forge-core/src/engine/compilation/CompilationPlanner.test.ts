...
describe('CompilationPlanner', () => {
  ...
  describe('buildPlan()', () => {
    ...
    it('should preserve hook grouping in forward outcomes', () => {
      ...
      context.nodeRegistry.register(journey.id, journey)
      context.nodeRegistry.register(step.id, step)
      context.nodeRegistry.register(redirect1.id, redirect1)
      context.nodeRegistry.register(redirect2.id, redirect2)
      context.astNodeTree.addNode(journey.id)
      context.astNodeTree.addNode(step.id, journey.id)
    ...
    it('should preserve a compilable hook when on its forward outcome group', () => {
      ...
      context.nodeRegistry.register(journey.id, journey)
      context.nodeRegistry.register(step.id, step)
      context.nodeRegistry.register(redirect.id, redirect)
      context.nodeRegistry.register(hookWhen.id, hookWhen)
      context.astNodeTree.addNode(journey.id)
      context.astNodeTree.addNode(step.id, journey.id)
