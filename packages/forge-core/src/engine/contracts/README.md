...
| [`reachability/`](./reachability/) | `ReachabilityEvaluation` and `JourneyReachabilityProjection` (the result of evaluating reachability at request time), plus the input/output types for the compiled reachability functions (`ReachabilityFactsInput`, `ReachabilityStateInput`, `ReachabilityEvaluationResult`) |
