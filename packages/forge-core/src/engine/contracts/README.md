...
| [`navigation/`](./navigation/) | `ReachabilityEvaluation` and `JourneyReachabilityState` (the result of evaluating reachability at request time), plus the input/output types for the compiled reachability function (`ReachabilityEvaluationInput` / `ReachabilityEvaluationResult`) |
