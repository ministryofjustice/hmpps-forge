...
The substrate itself is stage-neutral. Each stage owns its own tracer and projection:

- [`../compilation/tracing`](../compilation/tracing) records per-phase spans while a package compiles and projects them into a `CompilationTraceEvent`.
- [`../runtime/evaluation/request/RequestPipelineTraceProjector.ts`](../runtime/evaluation/request/RequestPipelineTraceProjector.ts) projects a finished request pipeline into a `RequestTraceEvent`.
