...
describe('createTestEffectContext', () => {
  ...
  it('should throw when mutating an object returned by getAnswer', () => {
    // Arrange
    const context = createTestEffectContext({ answers: { profile: { name: 'Ada' } } })

    // Act
    const profile = context.getAnswer<{ name: string }>('profile')
  ...
  it('should throw when mutating an array returned by getData', () => {
    // Arrange
    const context = createTestEffectContext({ data: { goals: ['first'] } })

    // Act
    const goals = context.getData<string[]>('goals')

    // Assert
    expect(() => goals.push('second')).toThrow(/read-only/)
  })
