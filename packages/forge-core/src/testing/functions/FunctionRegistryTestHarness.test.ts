...
describe('FunctionRegistryTestHarness', () => {
  ...
  describe('render()', () => {
    ...
    it('should reject an authored call whose component is not registered', async () => {
      ...
      const harness = new FunctionRegistryTestHarness(Registered)
    ...
    it('should distinguish separately registered components with the same name', async () => {
      // Arrange
      const FirstCard = component<object>('card', { factory: () => () => 'first' })
      const SecondCard = component<object>('card', { factory: () => () => 'second' })
      const harness = new FunctionRegistryTestHarness([FirstCard, SecondCard])

      // Act
      const firstOutput = await harness.render(FirstCard())
      const secondOutput = await harness.render(SecondCard())
